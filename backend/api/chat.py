...
def chat(
    ...
    try:
        ...
        if request.stream:
            def event_stream():
                try:
                    for chunk in orchestrator.stream_chat(session_id, request.message, **kwargs):
                        yield f"data: {json.dumps({'session_id': session_id, 'text': chunk}, ensure_ascii=False)}\n\n"
                    yield "data: [DONE]\n\n"
        ...
        response = orchestrator.chat(session_id, request.message, **kwargs)
