...
## Phase 19 — OCR Image Preprocessing\n- Autocontrast, contrast, sharpening and optional median denoise\n\n## Phase 20 — OCR Confidence + Quality Reports\n- Word-level confidence extraction\n- Average confidence score\n- High/medium/low quality classification\n\n## Phase 21 — OCR Language + Processing Controls\n- Configurable Bengali/English OCR language\n- Configurable preprocessing mode\n- Configurable minimum confidence threshold\n\n## Phase 22 — OCR/RAG Ingestion Hardening\n- Unified OCR result metadata for ingestion\n- Existing PDF text extraction remains preferred before OCR\n- Scanned-page OCR remains opt-in\n\n## Phase 23 — Unified Document AI Pipeline\n- OCR quality metadata flows with document processing\n- Release version `1.8.0`\n\n## Phase 18 — Bengali OCR + Scanned PDF RAG\n- OCR provider adapter with Tesseract support\n- Bengali + English OCR language support (`ben+eng`)\n- Scanned PDF detection: if normal PDF text extraction is empty, OCR can be enabled\n- PDF pages rendered for OCR using `pypdfium2`\n- OCR text enters the same RAG chunking/indexing/embedding pipeline\n- `POST /api/rag/documents` now supports `ocr_scanned=true` and `language=ben+eng`\n- Image OCR remains available at `POST /api/vision/ocr`\n- API release version `1.5.0`\n\n## Phase 17 — Source-Aware RAG + Document UI\n- Chat responses return retrieved document sources\n- Streaming chat emits a final source metadata event\n- Source entries include document ID, filename, score when available, and snippet\n- New document management/search UI at `/pwa/rag.html`\n- Existing RAG APIs remain compatible\n\n## Phase 16 — RAG-Augmented Chat\n- Chat automatically retrieves relevant document chunks\n- Retrieved context is injected into the model prompt without changing conversation history\n- Semantic retrieval is used when embeddings are configured\n- Keyword/FTS retrieval remains the fallback\n- RAG retrieval count is configurable with `GS420_RAG_TOP_K`\n- RAG can be disabled with `GS420_RAG_ENABLED=false`\n- API release integration keeps the existing `/api/chat` contract\n\n## Phase 15 — Advanced RAG + PDF/DOCX + Vector Search\n- PDF text extraction with `pypdf`\n- DOCX paragraph extraction with `python-docx`\n- Document chunking with overlap\n- Optional local Transformer embeddings\n- Semantic cosine-similarity search\n- Existing SQLite FTS5 search remains available\n- `GS420_EMBEDDING_MODEL_ID` enables semantic indexing\n- `POST /api/rag/documents` supports `embed=true`\n- `POST /api/rag/search` supports `semantic=true`\n- RAG statistics now include chunks and embedded chunks\n\n## Phase 14 — Production Observability + Monitoring\n- Request metrics and latency tracking\n- 5xx error counters\n- Recent request event buffer with request IDs\n- Structured exception logging\n- Admin-protected metrics API at `/api/metrics/summary`\n- Monitoring UI at `/pwa/monitor.html`\n- API release version `1.2.0`\n\n## Phase 14 — Production Observability + Monitoring\nPhase 13 — Management Dashboard + Admin Security\n- Admin dashboard at `/pwa/admin.html`\n- Token-protected management endpoints under `/api/admin`\n- `GS420_ADMIN_TOKEN` is never returned by the API\n- Redacted GS420 environment view\n- Runtime model/device/configuration snapshot\n- API release version `1.1.0`\n\n## Phase 12 — PWA + Management/Security Foundation
